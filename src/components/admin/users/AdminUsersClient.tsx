...
import { Loader2, MessageCircle, Pencil, Phone, MapPin, UserRound } from "lucide-react";
...
function EmptyCard() {
  return (
    <div className="bg-white rounded-lg shadow-[0px_1px_3px_0px_rgba(0,0,0,0.2)] p-5 flex flex-col items-center justify-center gap-3 h-48">
      <UserRound size={36} className="text-[#d1d5db]" />
      <p className="text-[14px] text-[#9ca3af] text-center leading-snug">
        Chọn khách hàng<br />để xem chi tiết
      </p>
    </div>
  );
}
...
function DetailCard({ user, orders, loading }: DetailCardProps) {
    ...
    <div className="bg-white rounded-lg shadow-[0px_1px_3px_0px_rgba(0,0,0,0.2)] p-5 flex flex-col gap-[10px]">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center gap-1 border border-[#e5e7eb] rounded-[4px] px-2 h-10">
          ...
          <span className="text-[14px] text-[#6a717f] truncate">Chưa có địa chỉ</span>
        </div>
      </div>
  ...
  return (
    <div className="p-6 flex gap-5 h-full min-h-0">
      ...
      <div className="w-[306px] shrink-0 pt-[68px]">
        {selectedUser ? (
          <DetailCard user={selectedUser} orders={userOrders} loading={ordersLoading} />
        ) : (
          <EmptyCard />
        )}
      </div>
