...
export default function StaffProductsReviewsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        while (!cancelled) {
          console.log("[StaffReviewsPage] Fetching tab:", activeTab, "page:", page);
          const res = await reviewService.getAllReviews(page, PER_PAGE, ratingFilter, accessToken);
        ...
        console.error("[StaffReviewsPage] Fetch error:", err);
      } finally {
        if (!cancelled) {
          setLoading(false);
  ...
  const ratingCounts = useMemo(() => {
    ...
    reviews.forEach((r) => {
      if (r.rating >= 1 && r.rating <= 5) counts[r.rating as 1 | 2 | 3 | 4 | 5]++;
    });
    return counts;
  }, [reviews]);
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-col overflow-hidden pr-2">
                    <span className="truncate text-gray-800 font-medium">{productName}</span>
                    {variantLabel && <span className="truncate text-xs text-gray-500">{variantLabel}</span>}
                  </div>
                  <div className="flex flex-col overflow-hidden pr-2">
                    <span className="truncate text-gray-800">{userName}</span>
                    {userEmail && <span className="truncate text-xs text-gray-500">{userEmail}</span>}
                  </div>
                  ...
                  <div className="text-gray-600 text-xs flex items-center gap-1">
                    {photoCount > 0 ? (
                      <><ImageIcon size={12} className="text-gray-500" />{photoCount}</>
                    ) : (
                      <span className="text-gray-300">—</span>
                    )}
                  </div>
                  <div className="text-gray-600 text-xs">{formatDate(review.createdAt)}</div>
                </div>
              );
            })}
          </div>
